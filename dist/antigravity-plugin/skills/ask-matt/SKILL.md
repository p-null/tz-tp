...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green slice at a time), then closes out by running **`/mskills:code-review`**, a two-axis review (Standards + Spec) of the diff, before committing (this repo's own `/code-review` is Claude Code's bundled bug-hunting reviewer, a different tool). Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/mskills:code-review`** on its own whenever you want to review a branch or PR against a fixed point.
